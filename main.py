...
parser.add_argument('--dataset_order', type=str, default='ETH', choices=dataset_choices)
...
train_params.add_argument('--batch_size', type=int, default=1024, help="batch-size")
train_params.add_argument('--optimizer', type=str, choices=['adam', 'adam_reset', 'sgd', 'sgdd'], default='adam')
...
train_params.add_argument("--gpu_index", default=1, type=int)
...
train_params.add_argument('--val_epoch', default=50, type=int, help="epoch start to validation")
...
replay_choices = ['der']
replay_params.add_argument('--replay', type=str, default='der', choices=replay_choices)
...
eval_params.add_argument('--metrics', action='store_true', default=True, help="calculate additional metrics (e.g., BWT, forgetting)")
eval_params.add_argument('--pdf', action='store_true', default=True, help="generator pdf with results")
...
batch_params.add_argument("--log_dir", default="save", help="Directory containing logging file")
...
def run(args, verbose=False):
    ...
    if args.method == "continual_learning":
        ...
        if verbose:
            print("\nPreparing the data...")
        #
        if args.dataset_order == 'ETH':
            train_order = ['ETH', 'UCY', 'inD', 'INTERACTION']
            val_order = ['ETH', 'UCY', 'inD', 'INTERACTION']
            test_order = ['ETH', 'UCY', 'inD', 'INTERACTION']
        if args.dataset_order == 'SDD':
            train_order = ['coupa', 'gates', 'hyang', 'nexus']
            val_order = ['coupa', 'gates', 'hyang', 'nexus']
            test_order = ['coupa', 'gates', 'hyang', 'nexus']
        ...
        if args.main_model == "lstm":
            from main_model.encoder0adc import Predictor
            model = Predictor(
                obs_len=args.obs_len,
                pred_len=args.pred_len,
                traj_lstm_input_size=args.traj_lstm_input_size,
        ...
        if args.main_model == "trans":
            from main_model.encoder import Predictor
            model = Predictor(obs_len=args.obs_len, pred_len=args.pred_len).to(device)
        if args.main_model == "gat":
            from main_model.encoder_gat import Predictor
            model = Predictor(
                obs_len=args.obs_len, pred_len=args.pred_len, traj_lstm_input_size=args.traj_lstm_input_size,
        ...
        elif model.optim_type == "sgdd":
            model.optimizer = torch.optim.SGD(
                model.parameters(),
                lr=args.lr,                # 初始学习率（建议0.01~0.1）
                momentum=0.8,              # 动量加速收敛[1,7](@ref)
                weight_decay=0             # 禁用L2正则化（与BatchNorm冲突）[8](@ref)
            )
        else:
        ...
        start = time.time()
        # Train model
        train_cl(args, best_ade, model, train_datasets, val_datasets, test_datasets, replay_model=args.replay, iters=args.iters, batch_size=args.batch_size,
        ...
        if args.pdf:
            ...
            BWT1 = 0
            for i in range(tasks):
                BWT1 +=metric_dict[key_ade]["task {}".format(i+1)][tasks-1] - metric_dict[key_ade]["task {}".format(i+1)][i]
            BWT1 /= (tasks - 1)

            metric_batch1 = [0.549625988627, 0.600105713238, 0.704164956503]
            FWT1 = 0
            for i in range(tasks - 1):
                FWT1 -=metric_batch1[i] - metric_dict[key_ade]["task {}".format(i+2)][i]
            FWT1 /= (tasks - 1)
            # FWT1 = 0
            # for i in range(tasks):
            #     FWT1 +=metric_dict[key_ade]["task {}".format(i+1)][0] - metric_dict[key_ade]["task {}".format(i+1)][i]
            # FWT1 /= (tasks - 1)
            ...
            BWT2 = 0
            for i in range(tasks):
                BWT2 +=metric_dict[key_fde]["task {}".format(i+1)][tasks-1] - metric_dict[key_fde]["task {}".format(i+1)][i]
            BWT2 /= (tasks - 1)

            metric_batch2 = [1.194354034289, 1.331612147715, 1.551430876359]
            FWT2 = 0
            for i in range(tasks - 1):
                FWT2 -=metric_batch2[i] - metric_dict[key_fde]["task {}".format(i+2)][i]
            FWT2 /= (tasks - 1)
            # FWT2 = 0
            # for i in range(tasks):
            #     FWT2 +=metric_dict[key_fde]["task {}".format(i+1)][0] - metric_dict[key_fde]["task {}".format(i+1)][i]
            # FWT2 /= (tasks - 1)
            ...
            # output
            output_file = open(
                "{}/2lossADE-FDE-{replay}-{iters}-{z_dim}-{batch_size}-{replay_batch_size}-{lr}-{aug}-{main_model}-{train_order}-{seed}_{val}_{val_class}_{si}_{si_c}.txt".format(args.r_dir, replay=args.replay,
                                                                                           iters=args.iters,
                                                                                           z_dim=args.z_dim,
                ...
                "w")
            output_file.write('ADEs:{ades}\nAverage_ADE:{ade}\nBWT:{BWT1}\nFWT:{FWT1}\nFDEs:{fdes}\nAverage_FDE:{fde}\nBWT:{BWT2}\nFWT:{FWT2}'.format(ades=plot_ade_list,ade=metric_dict["average_ade"],BWT1=BWT1,FWT1=FWT1,fdes=plot_fde_list,fde=metric_dict["average_fde"],BWT2=BWT2,FWT2=FWT2))
            output_file.close()
