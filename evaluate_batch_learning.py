...
parser.add_argument("--log_dir", default="save", help="Directory containing logging file")
...
def main(args):
    checkpoint_path = os.path.join(os.path.abspath(args.log_dir), "{}_{}_{}_best.pth.tar".format(args.main_model, args.log_dir, args.aug))
    checkpoint = torch.load(checkpoint_path)
    generator = get_generator(checkpoint)
    path = get_dset_path(args.dataset_name_test, args.dset_type)
    test_dset = data_dset(args, path)

    loader = data_loader(args, test_dset, 64)
